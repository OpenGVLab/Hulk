...
from core.data.datasets.images import smpl_data_tools
from torchvision.utils import make_grid
from .renderer import Renderer, visualize_reconstruction
from core.data.datasets.images.smpl_data_tools._smpl import SMPL, Mesh
...
class _SMPL_LOSS_FASTMETRO(nn.Module):
    ...
    def keypoint_3d_loss(self, criterion_keypoints, pred_keypoints_3d, gt_keypoints_3d, has_pose_3d, device='cuda'):
        ...
        """
        # conf = gt_keypoints_3d[:, :, -1].unsqueeze(-1).clone()
        gt_keypoints_3d = gt_keypoints_3d[:, :, :3].clone()
        gt_keypoints_3d = gt_keypoints_3d[has_pose_3d == 1]
        # conf = conf[has_pose_3d == 1]
        pred_keypoints_3d = pred_keypoints_3d[has_pose_3d == 1].clone()
        if len(gt_keypoints_3d) > 0:
    ...
    def forward(self, outputs, target):
        ...
        for i in range(5):
            rend_img = visualize_reconstruction(target['ori_img'][i].detach().cpu().numpy().transpose(1,2,0)/255, 224, gt_2d_joints[i].detach().cpu().numpy(), pred_3d_vertices_fine[i].detach().cpu().numpy(), pred_2d_joints_from_smpl[i].detach().cpu().numpy()[:,:2], outputs['cam_param'][i].detach().cpu().numpy().reshape((3,)), renderer) # cam[i].detach().cpu().numpy().reshape((3,))*0.5
            rend_img = rend_img.transpose(2,0,1)
            rend_imgs.append(torch.from_numpy(rend_img))
        rend_imgs = make_grid(rend_imgs, nrow=1)
        rend_imgs = rend_imgs.transpose(0,1)
        rend_imgs = rend_imgs.transpose(1,2)
        rend_imgs = np.asarray(rend_imgs)
        temp_fname = '/mnt/lustre/tangshixiang/hwz/smpl/unihcpv2/experiments/v2_smpl/vis/visual_ago_' + str(663) + '.jpg'
        cv2.imwrite(temp_fname, np.asarray(rend_imgs[:,:,::-1]*255))
        
        
        rend_imgs = []
        # Do visualization for the first 6 images of the batch
        for i in range(5):
            rend_img = visualize_reconstruction(target['ori_img'][i].detach().cpu().numpy().transpose(1,2,0)/255, 224, gt_2d_joints[i].detach().cpu().numpy(), pred_3d_vertices_fine[i].detach().cpu().numpy(), pred_2d_joints[i].detach().cpu().numpy()[:,:2], outputs['cam_param'][i].detach().cpu().numpy().reshape((3,)), renderer) # cam[i].detach().cpu().numpy().reshape((3,))*0.5
            rend_img = rend_img.transpose(2,0,1)
            rend_imgs.append(torch.from_numpy(rend_img))
        rend_imgs = make_grid(rend_imgs, nrow=1)
        rend_imgs = rend_imgs.transpose(0,1)
        rend_imgs = rend_imgs.transpose(1,2)
        rend_imgs = np.asarray(rend_imgs)
        temp_fname = '/mnt/lustre/tangshixiang/hwz/smpl/unihcpv2/experiments/v2_smpl/vis/visual_ago_tokens_' + str(663) + '.jpg'
        cv2.imwrite(temp_fname, np.asarray(rend_imgs[:,:,::-1]*255))
        
        rend_imgs = []
        # Do visualization for the first 6 images of the batch
        for i in range(5):
            rend_img = visualize_reconstruction(target['ori_img'][i].detach().cpu().numpy().transpose(1,2,0)/255, 224, gt_2d_joints[i].detach().cpu().numpy(), gt_vertices_fine[i].detach().cpu().numpy(), gt_2d_joints[i].detach().cpu().numpy()[:,:2], outputs['cam_param'][i].detach().cpu().numpy().reshape((3,)), renderer) # cam[i].detach().cpu().numpy().reshape((3,))*0.5
            rend_img = rend_img.transpose(2,0,1)
            rend_imgs.append(torch.from_numpy(rend_img))
        rend_imgs = make_grid(rend_imgs, nrow=1)
        rend_imgs = rend_imgs.transpose(0,1)
        ...
        temp_fname = '/mnt/lustre/tangshixiang/hwz/smpl/unihcpv2/experiments/v2_smpl/vis/visual_ago_gt_' + str(663) + '.jpg'
        cv2.imwrite(temp_fname, np.asarray(rend_imgs[:,:,::-1]*255))
